"""
FinSight Excel Generator - PAYMENTS FIX
Fixed: Cash Flow includes BOTH "Payment" and "Transfer" categories in inflows
"""
import os
...
def create_cash_flow_statement(wb, analysis_period: str = "Period"):
    """Create CASH FLOW STATEMENT tab - FIXED to include Payment category."""
    ...
    # FIXED: Include BOTH "Payment" and "Transfer" categories
    ws.cell(row, 1).value = "Payments & Transfers"
    ws.cell(row, 2).value = '=SUMIFS(Data!C:C,Data!E:E,"Payment",Data!C:C,">0")+SUMIFS(Data!C:C,Data!E:E,"Transfer",Data!C:C,">0")'
    ws.cell(row, 2).number_format = MONEY_FORMAT
    ws.cell(row, 3).value = f'=B{row}*12'
    ws.cell(row, 3).number_format = MONEY_FORMAT
    ws.cell(row, 4).value = "Money IN (payments, transfers)"
    ...
    ws.cell(row, 2).value = '=SUMIFS(Data!C:C,Data!E:E,"Refund",Data!C:C,">0")'
    ws.cell(row, 2).number_format = MONEY_FORMAT
    ws.cell(row, 3).value = f'=B{row}*12'
    ws.cell(row, 3).number_format = MONEY_FORMAT
    ws.cell(row, 4).value = "Returns and refunds"
    ws.cell(row, 4).font = Font(size=9, italic=True)
    ws.cell(row, 5).value = "ℹ️"
    ...
    # Other Credits
    ws.cell(row, 1).value = "Other Credits"
    ws.cell(row, 2).value = '=SUMIFS(Data!C:C,Data!E:E,"Entertainment",Data!C:C,">0")'
    ws.cell(row, 2).number_format = MONEY_FORMAT
    ws.cell(row, 3).value = f'=B{row}*12'
    ws.cell(row, 3).number_format = MONEY_FORMAT
    ws.cell(row, 4).value = "Other money received"
    ws.cell(row, 4).font = Font(size=9, italic=True)
    ws.cell(row, 5).value = "ℹ️"
